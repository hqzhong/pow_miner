...
const { argv } = yargs
    .options({
        ...
        donation: {
            description: 'Donate 5% of mined pow to the author',
            type: 'boolean',
            default: true,
        },
        min_balance: {
            description: 'min balance excute miner',
            type: 'number',
            default: 385,
        },
    })
...
const min_balance = argv.min_balance;
...
function get_currency_balance() {
    url = "https://api.eosbeijing.one/v1/chain/get_currency_balance";
    request.post(url, {
        json: { "code": "eosiopowcoin", "account": "eosiopowcoin", "symbol": "POW" }
    }, (error, res, body) => {
        if (error) {
            console.error(error);
            return
...
async function query_eidos_balance(account, rpc) {
    const balance_info = await rpc.get_currency_balance('eosiopowcoin', account, 'POW');
    const balance = parseFloat(balance_info[0].split(' ')[0]);
    return balance;
}
...
async function donate() {
    ...
    await send_eidos(account, 'prayxxzhqzyc', eidos_to_donate_str, 'donated from ' + account);
...
const CPU_RATE_EXPECTATION = 0.999; // we expect to keep CPU rate at 95%
const CPU_RATE_RED = 0.998; // Stop mining if CPU rate > 99.5%
const NUM_ACTIONS_MIN = 20;
const NUM_ACTIONS_MAX = 200;
let num_actions = NUM_ACTIONS_MAX;
...
function adjust_num_actions() {
    ...
    if (cpu_rate_ema_fast < CPU_RATE_EXPECTATION) {
        if (cpu_rate_ema_fast < 0.99) {
            num_actions = NUM_ACTIONS_MAX;
        } else if (cpu_rate_ema_fast < 0.995) {
            num_actions = 100;
        } else {
            num_actions = 100;
            // num_actions = Math.min(Math.ceil(num_actions * 4), NUM_ACTIONS_MAX);
        }
...
async function run() {
    try {
        ...
        const current_eidos_balance = await query_eidos_balance("eosiopowcoin", get_random_api().rpc, { fetch });
        if (current_eidos_balance < min_balance) {
            console.info(`current pow balance ${current_eidos_balance}, less then ${min_balance}, continue`);
            return;
        }
        console.info(`current pow balance: ${current_eidos_balance}`);
        ...
        if (increased != '0.0000' && !increased.startsWith('-')) {
            console.info(
                chalk.green('Mined ' + (current_balance - prev_balance).toFixed(4) + ' POW !!!'),
            );
        }
    } catch (e) {
        console.error(e);
...
(async() => {
    console.info(chalk.green(figlet.textSync('POW  Miner')));
    ...
    console.info(`POW balance: ${prev_eidos_balance}`);
    ...
    setInterval(run, 2000); // Mine EIDOS every second
